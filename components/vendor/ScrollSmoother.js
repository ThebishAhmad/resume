...
export var ScrollSmoother = /*#__PURE__*/function () {
    function ScrollSmoother(vars) {
        ...
        mainST = ScrollTrigger.create({
            ...
            onRefresh: function onRefresh(self) {
                ...
                if (recordedRefreshScroll) {
                    // if the refresh occurred mid-scroll (like the user resized the browser window while scrolled down), we should sync back up with the scroll position so that things don't jump.
                    ScrollTrigger.isTouch === 1 && (wrapper.style.position = "absolute");
                    wrapper.scrollTop = -recordedRefreshScroll; // works in Chrome, Firefox and Safari 16.

                    ScrollTrigger.isTouch === 1 && (wrapper.style.position = "fixed");

                    if (recordedRefreshScrub && scrub) {
                        scrub.progress(1);
                    }
                }
            ...
            // because all other pinned ScrollTriggers will affect the heights of things, so we must calculate this last.
            end: function end() {
        ...
        _updateDefaultScroller(mainST); // in case we used a default that's different than window


        vars.effects && this.effects(vars.effects, vars.effectsConfig);
        vars.sections && this.sections(vars.sections, vars.sectionsConfig);
    }
        ...
        if (!_coreInitted) {
            ...
            if (gsap) {
                ...
                _onResizeDelayedCall = gsap.delayedCall(0.2, function () {
                    return ScrollTrigger.isRefreshing || _mainInstance && _mainInstance.vars.autoResize !== false && resizeObserver.restart(true);
                }).pause();
                ScrollTrigger = gsap.core.globals().ScrollTrigger;
                gsap.core.globals("ScrollSmoother", ScrollSmoother); // must register the global manually because in Internet Explorer, functions (classes) that aren't defined in the global scope take on the name of the function itself, thus "ScrollSmoother" would be "ScrollSmoother".

                if (_body && ScrollTrigger) {
                    _getVelocityProp = ScrollTrigger.getVelocityProp;
                    _coreInitted = 1;
                }
            }
        }
    ...
    ScrollSmoother.create = function create(vars) {
        return _mainInstance && vars && _mainInstance.content() === _toArray(vars.content)[0] ? _mainInstance : new ScrollSmoother(vars);
    };

    ScrollSmoother.get = function get() {
...
ScrollSmoother.register = ScrollSmoother.register;

function _updateDefaultScroller(st) {
    var defaults = ScrollTrigger.defaults();
    defaults.scroller === _win && ScrollTrigger.defaults({
        scroller: _win === window ? document.body : _win
    });
    return st;
}
